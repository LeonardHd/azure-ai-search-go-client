...
// READ-ONLY; Contains debugging information specific to vector and hybrid search.
...
// QueryResultDocumentSubscores - The breakdown of subscores between the text and vector query components of the search query
// for this document. Each vector query is shown as a separate object in the same order they were received.
type QueryResultDocumentSubscores struct {
// READ-ONLY; The BM25 or Classic score for the text portion of the query.
	DocumentBoost *float64

// READ-ONLY; The BM25 or Classic score for the text portion of the query.
	Text *TextResult

// READ-ONLY; The vector similarity and @search.score values for each vector query.
	Vectors []map[string]*SingleVectorFieldResult
}
...
// READ-ONLY; Contains debugging information that can be used to further explore your search results.
...
// READ-ONLY; The relevance score computed by boosting the Reranker Score. Search results are sorted by the RerankerScore/RerankerBoostedScore
// based on useScoringProfileBoostedRanking in the Semantic Config.
...
// SingleVectorFieldResult - A single vector field result. Both @search.score and vector similarity values are returned. Vector
// similarity is related to @search.score by an equation.
type SingleVectorFieldResult struct {
// READ-ONLY; The @search.score value that is calculated from the vector similarity score. This is the score that's visible
// in a pure single-field single-vector query.
	SearchScore *float64

// READ-ONLY; The vector similarity score for this document. Note this is the canonical definition of similarity metric, not
...
// TextResult - The BM25 or Classic score for the text portion of the query.
...
type VectorsDebugInfo struct {
// READ-ONLY; The breakdown of subscores of the document prior to the chosen result set fusion/combination method such as
// RRF.
